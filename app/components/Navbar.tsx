...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <header className="fixed top-0 z-50 w-full backdrop-blur-xl bg-black/40 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
                ...
                <div className="absolute left-0 top-10 w-[520px] rounded-2xl bg-black/95 border border-white/10 shadow-2xl p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    {[
                      ...
                      <Link key={d.name} href={`/destinations/${d.name.toLowerCase().replace(' ', '-')}`} className="rounded-xl p-4 hover:bg-white/5 transition">
                        <p className="font-semibold text-white hover:text-[#d6b36a]">{d.name}</p>
          ...
          <div className="hidden md:flex items-center gap-4">
            <button onClick={() => setPropertyOpen(true)} className="rounded-full bg-black px-6 py-2 text-white hover:bg-gray-900 transition">
              List Your Property
            </button>

            <button onClick={() => setTripOpen(true)} className="rounded-full bg-[#d6b36a] px-6 py-2 font-semibold text-black">
              Plan a Trip
            </button>
          </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-sm">
          ...
          <nav className="px-6 py-8 space-y-6 text-lg text-white">
            ...
            <Link href="/stays" onClick={() => setMobileOpen(false)}>
              Stays
            </Link>
      ...
      {/* ================= PROPERTY MODAL ================= */}
      <ListPropertyModal open={propertyOpen} onClose={() => setPropertyOpen(false)} />
      <PlanTripModal open={tripOpen} onClose={() => setTripOpen(false)} />
    </>
  )
}
